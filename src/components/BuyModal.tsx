...
export default function BuyModal({ post, onClose }: BuyModalProps) {
  ...
  const message = `Добрый день! Я бы хотела купить ${postLink}. Количество: ${quantity || "не указано"}`;
  ...
  const contacts = [
    {
      ...
      getHref: () => {
        ...
        return `https://t.me/it_tkani_admin?text=${encoded}`;
      },
      icon: (
    ...
    },
    {
      ...
      getHref: () => {
        const encoded = encodeURIComponent(message);
        return `https://wa.me/79851858584?text=${encoded}`;
      },
      icon: (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
    ...
    },
    {
      name: "VK",
      getHref: () => {
        return `https://vk.com/bottega_tessile`;
      },
      icon: (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#e8e0d8]">
          <h3 className="font-heading text-2xl font-semibold text-[#2c2825]">
            {step === "quantity" ? "Укажите количество" : "Выберите способ связи"}
          </h3>
          <button
            onClick={onClose}
          ...
          {step === "quantity" ? (
            <form onSubmit={handleNext}>
              <p className="text-sm text-[#8a8178] mb-4">
                Сколько вы хотите купить? (например: 2м, 3 отреза)
              </p>
              <input
                type="text"
                value={quantity}
                onChange={(e) => setQuantity(e.target.value)}
                placeholder="Например: 2 метра"
            ...
            <>
              <p className="text-sm text-[#8a8178] mb-2">Товар:</p>
              <p className="text-sm font-medium text-[#2c2825] mb-1">{post.title}</p>
              <p className="text-sm text-[#8a8178] mb-4">Количество: {quantity}</p>

              <p className="text-sm text-[#8a8178] mb-4">
                Нажмите на удобный способ связи — сообщение будет сформировано автоматически:
              </p>
